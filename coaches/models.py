    ...
    def save(self, *args, **kwargs):
        if self.user.role not in [
            ...
            raise ValidationError(
                'User must be a coach or head coach.'
            )
...
class CoachAthleteAssignment(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.coach.role not in [
            'coach',
            'head_coach',
        ]:
            raise ValidationError(
                'Coach must have coach role.'
            )

        if self.athlete.role != 'athlete':
            raise ValidationError(
                'Athlete must have athlete role.'
            )
...
class CoachNote(models.Model):
    ...
    def save(self, *args, **kwargs):
        if self.athlete.role != 'athlete':
            raise ValidationError(
                'Note must be attached to an athlete.'
            )
    ...
    def __str__(self):
        return (
            f'Note for {self.athlete.username} '
            f'by {self.coach.username}'
        )
